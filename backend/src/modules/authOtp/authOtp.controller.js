...
async function sendOtp(req, res) {
  ...
  try {
    const result = await authOtpService.sendAuthOtp(msisdn);
...
async function verifyOtp(req, res) {
  ...
  try {
    const result = await authOtpService.verifyAuthOtp(msisdn, otp);
...
// Unsubscribe — koi cookie/logout logic yahan nahi, sirf telecom cancel
async function UnsubscribeUser(req, res) {
  ...
  try {
    const result = await authOtpService.unsubscribeUser(msisdn);
