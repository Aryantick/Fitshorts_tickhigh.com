...
/**
 * Jordan Orange (Beecell Gateway) Provider Implementation
 * Multi-tenant fitness reels platform adapter for Jordan (subdomain: 'orjo')
 * 
 * Supports:
 * 1. GET  /beecell/api/users/info (checksub)
 ...
 */
class JordanOrangeProvider {
  constructor(config) {
    if (!config || !config.base_url) {
      throw new Error("JordanOrangeProvider requires base_url in telecom config");
    }

    const extra = config.extra_config || {};
    this.baseUrl = config.base_url;
    this.serviceId = extra.serviceId || "13080";
    this.subdomain = config.subdomain || "orjo";
    this.clientId = config.client_id || 5;
  ...
  async checkSub(msisdn) {
    try {
      ...
      console.error("Jordan Orange checkSub error:", error.message);
      return {
        responseCode: "0",
        data: {
          currentStatus: "unsub",
  ...
  async subscribeOtp(msisdn, subServiceId = null) {
    try {
      ...
      console.error("Jordan Orange subscribeOtp error:", error.message);
   ...
   * Payload: { "msisdn": "962771234567", "code": "12345" }
   */
  async validateOtp(msisdn, otp) {
    try {
      ...
      console.error("Jordan Orange validateOtp error:", error.message);
      const resData = error.response?.data || {};
      return {
        responseCode: String(resData.responseType || "1"),
        status: "failed",
  ...
  /**
   * Subscription Plan Sync (Operator charging engine sync if required)
   */
...
module.exports = JordanOrangeProvider;
